//! # VENDORED FILE — keep in sync with upstream.
//!
//! This is a verbatim copy of `ffi/src/lib.rs` from the
//! [`momiji-rs/sasso`](https://github.com/momiji-rs/sasso) repository. It is
//! vendored here (rather than referenced via a path dependency) so that the
//! `sasso-python` package builds against the **published** `sasso` core crate
//! straight from crates.io, exactly as a downstream consumer would.
//!
//! Do NOT edit this file in isolation: any change to the C ABI must land first
//! in `momiji-rs/sasso` `ffi/src/lib.rs` (and `ffi/include/sasso.h`), then be
//! re-copied here, and the matching `include/sasso.h` + `src/sasso/_ffi.py`
//! struct layouts updated in lockstep. The `version = "=0.6.3"` pin on the core
//! crate in `native/Cargo.toml` anchors which ABI this copy targets.
//!
//! ---
//!
